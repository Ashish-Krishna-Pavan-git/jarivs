...
def init_db() -> None:
    ...
    with _lock, _connect() as db:
        ...
        for version, name, migration in MIGRATIONS:
            ...
            try:
                migration(db)
                db.execute("INSERT INTO schema_migrations(version,name,applied_at) VALUES(?,?,?)", (version, name, datetime.utcnow().isoformat()))
...
def ensure_admin_user(make_hash) -> None:
    ...
    with _connect() as db:
        if db.execute("SELECT id FROM users WHERE username=?", (username,)).fetchone():
            return
    with _lock, _connect() as db:
        db.execute("INSERT INTO users(username,password_hash,role,display_name,must_change_password,active,created_at) VALUES(?,?,'admin','Administrator',?,1,?)", (username, make_hash(password), must_change, datetime.utcnow().isoformat()))
    if must_change:
...
def log_event(level: str, component: str, message: str, details: dict[str, Any] | None = None) -> None:
    try:
        with _lock, _connect() as db:
            db.execute("INSERT INTO event_logs(level,component,message,details_json,created_at) VALUES(?,?,?,?,?)", (level.upper(), component, message, json.dumps(details or {}), datetime.utcnow().isoformat()))
    except Exception:
        pass
...
def block_model_provider(name: str, seconds: int, reason: str) -> None:
    until = (datetime.utcnow() + timedelta(seconds=max(int(seconds), 1))).isoformat()
    with _lock, _connect() as db:
        db.execute("UPDATE model_providers SET blocked_until=?,updated_at=? WHERE name=?", (until, datetime.utcnow().isoformat(), name))
...
def disable_notification_channel(channel_id: int, user_id: int | None = None) -> None:
    sql, params = "UPDATE notification_channels SET enabled=0,updated_at=? WHERE id=?", [datetime.utcnow().isoformat(), int(channel_id)]
    if user_id is not None:
        sql += " AND user_id=?"
        params.append(int(user_id))
    with _lock, _connect() as db:
...
def record_migration(source_path: str, kind: str, item_key: str, status: str, details: dict[str, Any] | None = None) -> None:
    with _lock, _connect() as db:
        db.execute("""INSERT INTO migration_records(source_path,kind,item_key,status,details_json,created_at) VALUES(?,?,?,?,?,?)
            ...
            (source_path, kind, item_key, status, json.dumps(details or {}), datetime.utcnow().isoformat()))
