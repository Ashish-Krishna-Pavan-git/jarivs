...
**Latest Milestone (2026-08-05):**
- ✅ **Durable Persistence Engine**: Updated database seeding to verify table counts before inserting template defaults. Added SQLite database `data/jarvis.db` to Hugging Face Dataset sync (`CRITICAL_FILES`). Sources, channels, model providers, and configuration changes remain permanent across restarts, rebuilds, and reloads.
- ✅ **Telegram Toggle & Complete Isolation**: Telegram defaults OFF on Hugging Face Spaces. When disabled, zero network requests to `api.telegram.org` execute. Slack delivery operates independently.
- ✅ **Slack Upload V2 API**: Replaced deprecated `files.upload` endpoint with Slack's modern 3-step upload API (`files.getUploadURLExternal` -> POST binary `upload_url` -> `files.completeUploadExternal`).
- ✅ **Built-in MCP Source Audit**: Multi-threaded RSS feed auditor (`mcp_audit_sources()`) evaluating reachability, latency, freshness (>72h), and duplicate rates concurrently.
- ✅ **True One-Button Factory Reset**: Performs clean runtime wipe (`raw_articles/`, `processed/`, `daily/`, `archive/`, `audio/`, `images/`, `podcasts/`, `drafts/`, `reports/`, `cache/`, `seen.json`, `digest_state.json`, `telemetry.json`, `runtime_state.json`, `wordpress_posts.jsonl`, in-memory queue, dedupe cache, `event_logs`) and syncs clean state to HF Dataset while preserving users, sources, models, and channels.
- ✅ **Enhanced Admin Observability**: Upgraded Models, Mcp, Logs, Channels, and Health pages with real-time status badges, component filters, export tools, and clear error diagnostics.
