...
- ✅ **True Factory Reset**: Enhanced `/api/admin/factory-reset` to perform a complete wipe of all runtime data (`raw_articles/`, `processed/`, `daily/`, `archive/`, `data/audio/`, `data/images/`, `data/podcasts/`, `data/drafts/`, `seen.json`, `digest_state.json`, `telemetry.json`, `runtime_state.json`, `wordpress_posts.jsonl`, in-memory queue, dedupe cache, `event_logs`). Displays `Processed=0, Scraped=0, Cycles=0, Queue=0, Reports=0, Audio=0, Phase=Idle, Last Cycle=Never` while preserving users, sources, models, and channels.
- ✅ **Redesigned Severity Engine**: Implemented `evaluate_severity()` in `ai_router.py` with strict classification criteria for `CRITICAL`, `HIGH`, `MEDIUM`, `LOW`, `MINIMAL`, 0-100 confidence scoring, deterministic capping against over-classification, and detailed `(severity, confidence, reason)` logging.
