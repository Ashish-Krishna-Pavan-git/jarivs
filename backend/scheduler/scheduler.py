...
def run_cycle(slot_label, boot_cycle=False):
    ...
    try:
        ...
        if processed:
            try:
                from ai_router import ai_digest, get_ai_status
                digest_data = ai_digest(processed, slot_label)
                ai_status_info = get_ai_status()
            except Exception as e:
                print(f"{prefix} Digest AI error: {e}")
                try:
        ...
        # Always save a digest record so the Reports page shows real content,
        # even when AI synthesis is unavailable (degraded mode).
        if processed:
            try:
                from storage import save_digest
                if not digest_data:
                    # Build a minimal digest from processed items so Reports is never empty.
                    digest_data = {
                        "headline": f"Cycle {n} — {len(processed)} articles processed (AI synthesis unavailable)",
                        "cybersec_updates": [str(i.get("title","")) for i in processed if i.get("category") == "cybersec"][:5],
                        "ai_updates": [str(i.get("title","")) for i in processed if i.get("category") == "ai"][:5],
                        "tech_business_updates": [str(i.get("title","")) for i in processed if i.get("category") in ("tech","business")][:5],
                        "hardware_mobile_updates": [str(i.get("title","")) for i in processed if i.get("category") in ("hardware","mobile")][:5],
                        "key_cves": list({c for i in processed for c in (i.get("cves") or [])})[:10],
                        "strategic_note": "AI synthesis was unavailable for this cycle. Showing top items by category. Check Admin → Logs for AI errors and Admin → Models for provider status.",
                        "_degraded": True,
                    }
                save_digest(digest_data, n)
                print(f"{prefix} Digest saved (cycle {n})")
            except Exception as e: print(f"{prefix} Digest save: {e}")

        # Update runtime state with AI + notification status for UI visibility
        try:
            from ai_router import get_ai_status as _get_ai
            _state(ai_status=_get_ai())
        except Exception:
            pass
